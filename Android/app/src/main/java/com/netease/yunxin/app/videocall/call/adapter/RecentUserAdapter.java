...
public class RecentUserAdapter extends RecyclerView.Adapter<RecentUserAdapter.ViewHolder> {
  ...
  private final List<LoginInfo> mUsers = new ArrayList<>();
  ...
  public void updateUsers(List<LoginInfo> users) {
    if (users == null) {
      return;
    }
    this.mUsers.clear();
  ...
  public void updateItem(LoginInfo user) {
    if (user == null) {
      return;
    }
    this.mUsers.clear();
  ...
  public void onBindViewHolder(ViewHolder holder, int position) {
    LoginInfo searchedUser = mUsers.get(position);
    holder.tvNickname.setText(mUsers.get(position).getAccount());
    //    Glide.with(mContext)
