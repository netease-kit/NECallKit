...
public class SettingActivity extends AppCompatActivity {
  ...
  /** AI降噪模式 */
  public static NERtcAudioAINSMode AINS_MODE = NERtcAudioAINSMode.kNERtcAudioAINSOff;
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    // 初始化AI降噪模式Spinner
    Spinner spinnerAINSMode = findViewById(R.id.spinner_ains_mode);
    String[] ainsModeOptions = {
      getString(R.string.ains_mode_off),
      getString(R.string.ains_mode_normal),
      getString(R.string.ains_mode_enhance)
    };
    ArrayAdapter<String> ainsAdapter =
        new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, ainsModeOptions);
    ainsAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    spinnerAINSMode.setAdapter(ainsAdapter);

    // 从SharedPreferences读取保存的值
    int savedAINSMode =
        SPUtils.getInstance().getInt("ainsMode", NERtcAudioAINSMode.kNERtcAudioAINSOff.ordinal());
    AINS_MODE = NERtcAudioAINSMode.values()[savedAINSMode];
    spinnerAINSMode.setSelection(savedAINSMode);

    findViewById(R.id.bt_upload_rtc_log)
        .setOnClickListener(
            v -> {
              NERtcEx.getInstance().uploadSdkInfo();
              Toast.makeText(SettingActivity.this, R.string.upload_success, Toast.LENGTH_SHORT)
                  .show();
            });
        ...
        .setOnClickListener(
            v -> {
              try {
                ...
                // 保存并应用AI降噪模式设置
                int selectedAINSMode = spinnerAINSMode.getSelectedItemPosition();
                AINS_MODE = NERtcAudioAINSMode.values()[selectedAINSMode];
                SPUtils.getInstance().put("ainsMode", selectedAINSMode);
                NECallEngine.sharedInstance().setAINSMode(AINS_MODE);
                ...
                Toast.makeText(SettingActivity.this, R.string.settings_success, Toast.LENGTH_SHORT)
                    .show();
                finish();
              } catch (Exception e) {
                Toast.makeText(SettingActivity.this, R.string.settings_failed, Toast.LENGTH_SHORT)
                    .show();
              }
            });
