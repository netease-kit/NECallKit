      ...
      btJoin.setOnClickListener(
          v -> {
            ...
            NimSignallingWrapper.joinRoom(
                params,
                new Callback<V2NIMSignallingJoinResult>() {
                  @Override
                  public void onResult(int code, String msg, V2NIMSignallingJoinResult result) {
                    if (result != null
                        && result.getRoomInfo() != null
                        && result.getRoomInfo().getChannelInfo() != null) {
                      inputLog(
                          "join:"
                              + code
                              + ",channelId-"
