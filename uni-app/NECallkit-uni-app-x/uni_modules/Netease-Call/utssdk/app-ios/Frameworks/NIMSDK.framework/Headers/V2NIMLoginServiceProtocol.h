...
/// 检查多端互斥情况。默认NO。当开启次选项时，如果参数forceMode = NO，同时上次登录错误码是V2NIM_ERROR_CODE_MULTI_LOGIN_FORBIDDEN，或者登录期间被踢下线，登录返回错误码V2NIM_ERROR_CODE_MULTI_LOGIN_FORBIDDEN或V2NIM_ERROR_CODE_LAST_KICKED。此时offlineMode参数无效，也无法进入离线默认。需要打开forceMode = YES才能继续登录。
@property(nonatomic,assign) BOOL checkClientExclusive;
...
/// 连接选项
@property(nullable,nonatomic,strong) V2NIMConnectOption *connectOption;
...
/// 连接选项
@interface V2NIMConnectOption : NSObject

/// 登录后重置重连延迟
@property(nonatomic,assign) BOOL resetReconnectDelayAfterLogin;

/// 最大等待网络时间，单位s，默认30s，如果设置为 <= 0则禁用，将完全依赖网络变更通知发起连接
@property(nonatomic,assign) NSInteger maxWaitNetworkInterval;
