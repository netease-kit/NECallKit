 ...
 *  @param failedImportedRecentSessions  导入失败的最近会话
 */
typedef void(^NIMImportServerRecentSessionsBlock)(NSError * __nullable error, NSArray<NIMRecentSession *> * __nullable failedRecentSessions);
 ...
 *  @param error  错误,如果成功则error为nil
 */
typedef void(^NIMDeleteAllMessagesInSessionsBlock)(NSError * __nullable error, NSArray<NIMSession *> * __nullable sessions);
 ...
 *  @param error  错误,如果成功则error为nil
 */
typedef void(^NIMDeleteAllMessagesInSessionBlock)(NSError * __nullable error);
...
- (void)deleteAllMessagesInSession:(NIMSession *)session
                            option:(nullable NIMDeleteMessagesOption *)option
                        completion:(nullable NIMDeleteAllMessagesInSessionBlock)block;
 ...
 *  @param option 删除消息选项
 */
- (void)deleteAllMessagesInSessions:(nonnull NSArray<NIMSession *> *)sessions
                            option:(nullable NIMDeleteMessagesOption *)option
                        completion:(nullable NIMDeleteAllMessagesInSessionsBlock)block;
 ...
 *  @discussion 当导入最近会话成功之后，不会收到 NIMChatManagerDelegate 中的 recentSession 变化的回调，请直接在 completion 中做处理。不允许插入已经存在的最近会话。
 */
- (void)importServerRecentSessions:(NSArray<NIMRecentSession *> *)recentSessions
                  completion:(nullable NIMImportServerRecentSessionsBlock)completion;
