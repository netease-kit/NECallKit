...
typedef NS_ENUM(int, NERtcError) {
    ...
    */
     ...
     * @endif
     */
    kNERtcErrFeatureSupportedHardwareLimit = 30031,
     ...
     * @if Chinese
     * 媒体流转发状态无效。
     * <br>原因通常为重复调用 startChannelMediaRelay。成功调用 startChannelMediaRelay 后，必须先调用 stopChannelMediaRelay 方法退出当前的转发状态，才能再次调用该方法。
     * @endif
     */
     ...
     * 播放低延时直播流失败
     */
    kNERtcErrPlayLowLatencyStreamFailed = 30114,
    /**
     * 播放低延时直播流超时
     */
    kNERtcErrPlayLowLatencyStreamTimeout = 30115,
    /**
     * @if English
     ...
     * @endif
     */
    kNERtcRuntimeErrVDMScreenSourceTypeError = 50401,
    /**
     * @if English
     * @endif
     * @if Chinese
     ...
     * @endif
     */
    kNERtcRuntimeErrScreenAudienceState = 50402,
  
    /**
     * @if Chinese
     * 视频采集中断。由于系统压力（如资源过载、温度过高等）导致视频设备不可用。
     * 这种中断通常发生在系统资源紧张的情况下，例如当设备正在执行多个高资源消耗的任务时，系统可能会限制摄像头的使用，以保护设备不被过热或过度消耗电池等。
     * @endif
     */
    kNERtcRuntimeErrVDMCameraDueToSystemPressure = 51005,
    /**
     * @if English
     * Live play stream id conflict
     * @endif
     * @if Chinese
     * 直播拉流流ID冲突。
     * @endif
     */
    kNERtcLivePlayStreamIdConflict = 70000, /** stream id冲突 */
    /**
     * @if English
     * Live play stream id not found
     * @endif
     * @if Chinese
     * 直播拉流流ID未发现。
     * @endif
     */
    kNERtcLivePlayStreamIdNotFound = 70001, /** stream id未发现 */
    /**
     * @if English
     * Live play connect failed
     * @endif
     * @if Chinese
     * 直播拉流连接失败。
     * @endif
     */
    kNERtcLivePlayConnectFailed = 70002, /** 直播拉流连接失败 */
    /**
     * @if English
     * Live play reconnect failed
     * @endif
     * @if Chinese
     * 直播拉流重连失败。
     * @endif
     */
    kNERtcLivePlayReconnectFailed = 70003, /** 直播拉流重连失败 */
    /**
     * @if English
     * Live play invalid address
     * @endif
     * @if Chinese
     * 直播拉流无效地址。
     * @endif
     */
    kNERtcLivePlayInvalidAddress = 70004, /** 直播拉流无效地址 */
    /**
     * @if English
     * Live play core source lost
     * @endif
     * @if Chinese
     * 直播拉流引擎资源缺失。
     * @endif
     */
    kNERtcLivePlayCoreSourceLost = 70005, /** 直播拉流引擎资源缺失 */
    /**
     * @if English
     * Live play audio start failed
     * @endif
     * @if Chinese
     * 直播拉流音频启动失败。
     * @endif
     */
    kNERtcLivePlayAudioStartFailed = 70006, /** 直播拉流音频启动失败 */
    /**
     * @if English
     * Live play video start failed
     * @endif
     * @if Chinese
     * 直播拉流视频启动失败。
     * @endif
     */
    kNERtcLivePlayVideoStartFailed = 70007, /** 直播拉流视频启动失败 */
    /**
     * @if English
     * Not in live play streaming
     * @endif
     * @if Chinese
     * 不在直播拉流状态。
     * @endif
     */
    kNERtcLivePlayNotInPlayStreaming = 70008, /** 不在直播拉流状态 */
};
