 ...
 * @param uid 用户 ID。如果在 joinChannel 方法中指定了 uid，此处会返回指定的 ID; 如果未指定 uid（joinChannel 时 uid=0），此处将返回云信服务器自动分配的 ID。
 * @endif
 */ 
 ...
 * context.engineDelegate = delegate; //delegate 处理对象        
 * NERtcLogSetting *logSetting = [[NERtcLogSetting alloc] init];
 * logSetting.logDir = logDir;//日志路径
 * logSetting.logLevel = kNERtcLogLevelWarning; //debug 级别可选择 kNERtcLogLevelInfo 或者更高，有利于调试中的问题定位;release 版本建议用 warning 级别
...
/**
   * 设置 SDK 主回调，调用此方法后会更新之前 setupEngineWithContext 及 setEngineEventDelegate 设置的回调。
   * 如果设置为空指针， 那么表示清空回调。
 ...
 * - 如果您指定了自己的 delegateQueue，请不要直接 NERtcEngineDelegateEx 回调函数中操作 UI，会引发线程安全问题。且需要考虑线程的生命周期，如果线程提前终止，将无法收到 NERtcEngineDelegateEx 的回调。
 */
 ...
 *      <td>token</td>
 *      <td>NSString</td>
 *      <td>安全认证签名（NERTC Token），可以设置为：<ul><li>null。调试模式下可设置为 null。安全性不高，建议在产品正式上线前在网易云信控制台中将鉴权方式恢复为默认的安全模式。<li>已获取的 NERTC Token。安全模式下必须设置为获取到的 Token。若未传入正确的 Token 将无法进入房间。推荐使用安全模式。</td>
 *  </tr>
 *  <tr>
 *      <td>channelName</td>
 *      <td>NSString</td>
 ...
 *      <td>token</td>
 *      <td>NSString *</td>
 *      <td>在服务器端生成的用于鉴权的安全认证签名（Token），可设置为：<ul><li>已获取的 NERTC Token。安全模式下必须设置为获取到的 Token，默认 Token 有效期为 10min，也可以定期通过应用服务器向云信服务器申请 Token 或者申请长期且可复用的 Token。<li>null。调试模式下可设置为 null。安全性不高，建议在产品正式上线前在网易云信控制台中将鉴权方式恢复为默认的安全模式。</td>
 *  </tr>
 *  <tr>
 *      <td>channelName</td>
 *      <td>NSString *</td>
 ...
 * - 调用此函数场景类型为视频 1 对 1 呼叫场景、清晰度较高的 1 对 1 呼叫场景等 v5.5.40 新增的房间场景将覆盖您通过 {@link setLocalVideoConfig}, { @link  setAudioProfile} 等 API 设置的音视频相关配置，因此建议先第一时间设置场景再通过其他 API 调整音视频配置。
 * - V5.5.40 之前就存在通信场景和直播场景，设置通信场景和直播场景，不会覆盖  {@link setLocalVideoConfig}, { @link  setAudioProfile} 等 API 设置的音视频相关配置
 * @par 调用时机
 * 请在初始化后调用该方法，且该方法仅可在加入房间前调用。建议在初始化之后先调此方法，再调别的 API 设置的音视频相关配置。
 * 加入房间后, 只有语聊房场景、高品质语聊房场景、卡拉 OK (KTV) 场景之间可以互相切换，其他房间场景会中无法切换。
 * @par 参数说明
 * <table>
 *  <tr>
 *      <th>**参数名称**</th>
 ...
 *      <td>int</td>
 *      <td>设置房间场景：<ul><li>kNERtcChannelProfileCommunication：通话场景。<li>kNERtcChannelProfileLiveBroadcasting：直播场景。<li>kNERtcChannelProfileVideoCall：视频 1 对 1 呼叫场景。 <li>kNERtcChannelProfileHighQualityVideoCall：清晰度较高的 1 对 1 呼叫场景。 <li>kNERtcChannelProfileChatroom：语聊房场景。<li>kNERtcChannelProfileHighQualityChatroom：高品质语聊房场景。<li>kNERtcChannelProfileMeeting：会议场景。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 ...
 * //设置 profile 为标准模式，scenario 为语音场景
 * NERtcAudioProfileType profile = kNERtcAudioProfileStandard;
 * [[NERtcEngine sharedEngine] setAudioProfile:profile];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @if Chinese
 * 设置音频应用场景
 * <br>通过此接口可以实现设置音频属性的应用场景，包括聊天室场景、语音场景、音乐场景。也可以使用 通过 {@link INERtcEngine#setChannelProfile:} 设置其他场景，包括 1v1 呼叫场景、清晰度较高的 1v1 呼叫场景、语聊房场景、清晰度较高的语聊房场景、会议场景。
 * @since V5.5.40
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法在加入房间前后均可调用。
 * @par 参数说明
 ...
 * //设置 scenario 为语音场景
 * NERtcAudioScenarioType scenario = kNERtcAudioScenarioSpeech;
 * [[NERtcEngine sharedEngine] setAudioScenario:scenario];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @code
 * //设置 AI 降噪为普通模式
 * NERtcAudioAINSMode mode = kNERtcAudioAINSNormal;
 * [[NERtcEngine sharedEngine] setAINSMode:mode];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * 设置音视频通话的相关参数。
 * <br>
 * 此接口提供技术预览或特别定制功能，详情请 [提交工单](https://app.yunxin.163.com/global/service/ticket/create) 联系网易云信技术支持工程师。
 * @since V3.5.0
 * @par 调用时机  
 * 请在初始化后调用该方法，且该方法在加入房间前后均可调用。
 * @par 参数说明
...
/**
  以 String 的形式获取一些内部参数，此接口为隐藏接口，需要特定参数及特定时机，详情请 [提交工单](https://app.yunxin.163.com/global/service/ticket/create) 联系网易云信技术支持工程师。
 
  @param parameterKey 参数 key
  @param extraInfo    额外的信息
  @return 如果查询到相关参数，以 String 形式返回，否则返回 nil
 */
