 ...
 * @par 示例代码
 * @code
 * [channel setVideoStreamLayerCount:kNERtcVideoStreamLayerCountTwo];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @endif
 */
- (int)setupRemoteVideoCanvas:(NERtcVideoCanvas * _Nullable)canvas forUserID:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType;
 ...
 * @endcode 
 * @par 相关回调
 * 订阅远端视频流后，远端会收到 {@link NERtcChannelDelegate#onNERtcChannelUser:videoMuted:streamType:} 回调。
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 *      - 30001（kNERtcErrFatal）：引擎尚未初始化。
...
/**
 * 引擎 3D 音效算法距离范围设置
 * @note 依赖 enableSpatializer:applyToTeam 接口开启，通话前调用
...
/**
     ...
     * @since V5.4.101
     * @param[in] type 音频流类型。
     * @param[in] uid_array 不订阅此 用户 uid 列表 的音频。
