 ...
 * ## 错误码
 * 
 * 在调用 SDK API 的过程中，SDK 可能会返回错误码或状态码，您可以根据错误码或状态码判断当前 SDK 或任务的状态。如果遇到未知的错误码，请 [提交工单](https://app.yunxin.163.com/global/service/ticket/create) 联系网易云信技术支持工程师排查。
 * 
 * 当前 SDK API 的错误码如下，各类错误码页面顶部可以查看对应的错误码值。
 * - 通用错误码：{@link NERtcError}
 * - 伴音错误码：{@link NERtcAudioMixingErrorCode}
 * - 客户端录音状态码：{@link NERtcAudioRecordingCode}
 * - 视频水印状态码：{@link NERtcLocalVideoWatermarkState}
...
<table>
 ...
 <table>
  ...
  <tr>
    ...
    <td>查询当前设备是否支持开启指定功能（例如虚拟背景功能）。</td>
    <td>V5.6.30</td>
  </tr>
</table>
...
|---|---|:---:|
| {@link INERtcEngineEx#startPushStreaming: } | 将 RTMP 流推到指定的地址。 | V4.6.420 |
| {@link INERtcEngineEx#stopPushStreaming: } | 停止当前正在进行的 RTMP 推流。 | V4.6.420 |

| <div style="width:400px">事件</div> | <div style="width:400px">描述</div> | <div style="width:200px">起始版本</div> |
|---|---|:---:|
| {@link NERtcEngineDelegateEx#onNERtcEngineStartPushStreamingWithResult: } | startPushStreaming 开始推流的结果回调。 | V4.6.420 |
| {@link NERtcEngineDelegateEx#onNERtcEngineStopPushStreaming: } | 停止 RTMP 推流的结果回调。 | V4.6.420 |
| {@link NERtcEngineDelegateEx#onNERtcEnginePushStreamingChangeToReconnectingWithReason: } | 推流过程中推流状态变为重连中的回调。 | V4.6.420 |
| {@link NERtcEngineDelegateEx#onNERtcEnginePushStreamingReconnectedSuccess: } | 推流过程中重连成功的回调。 | V4.6.420 |
...
<table>
  ...
  <tr>
    <td> {@link INERtcEngineEx#stopAudioDumpWithType:} </td>
    <td>结束记录音频 dump</td>
    <td>V4.6.0</td>
  </tr>
  <tr>
