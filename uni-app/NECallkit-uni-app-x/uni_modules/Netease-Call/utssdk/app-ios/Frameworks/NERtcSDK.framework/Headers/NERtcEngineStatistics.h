...
NERTC_EXPORT  @interface NERtcVideoSendStats : NSObject
  ...
  * @endif
  * @if Chinese
  * 具体每条流的上行统计信息 参考 NERtcVideoLayerSendStats
  * @endif
  */
...
NERTC_EXPORT  @interface NERtcVideoLayerRecvStats : NSObject
 ...
 /**
  * @if English 
  * The stream type: Bigstream (1), Substream (2) Thirdstream (3), Fourthstream (4).
  * @endif
  * @if Chinese
  * 流类型： 主流（1）， 辅流（2），第三流（3）， 第四流（3）
  * @endif
 ...
 * @endif
 * @if Chinese
 * 当前 uid 每条流的接收下行统计信息 参考 NERtcVideoLayerRecvStats
 * @endif
 */
...
-(void)onRtcStats:(NERtcStats *)stat;
  ...
  * @param stat 本地音频流统计信息。参考 NERtcAudioSendStats.
  * @endif
  */
- (void)onLocalAudioStat:(NERtcAudioSendStats *)stat;
  ...
  * @param stats 每个远端用户音频统计信息的数组。参考 NERtcAudioRecvStats.
  * @endif
  */
- (void)onRemoteAudioStats:(NSArray<NERtcAudioRecvStats*> *)stats;
  ...
  * @param stat 本地视频流统计信息。参考 NERtcVideoSendStats.
...
-(void)onLocalVideoStat:(NERtcVideoSendStats *)stat;
  ...
  * @param stats 远端用户视频统计信息的数组。参考 NERtcVideoRecvStats.
  * @note 纯音频 SDK 禁用该接口，如需使用请前往云信官网下载并替换成视频 SDK
  * @endif
  */
