...
/**
 * @if English
 * BOOL - Specifies AI-powered noise reduction mode. If you enable AI-powered noise reduction, the peer can hear your voice in a noisy environment.
 * @endif
 * @if Chinese
 * 设置 AI 降噪模式。已废弃，请使用 setAINSMode 接口代替。
 ...
 * <p>
 * note:需要在初始化前设置，释放 SDK 后失效。一般配合 {@link INERtcEngineEx#setPreDecodeObserver:} 使用。
 * @since 4.6.25
 */
 ...
 * @endif
 * @if Chinese
 * 是否使用外部渲染器，默认 NO 不使用，若使用 SDK 内部会根据 externalVideoRender 来进行绑定，若不使用 SDK 内部根据 container 来进行绑定。暂不支持在通话中切换内部渲染与外部渲染。
 * @endif
 */
 ...
 * @if Chinese
 * 视频镜像模式。
 * - 本地视图镜像模式：如果您使用前置摄像头，默认启动本地视图镜像模式；如果您使用后置摄像头，默认关闭本地视图镜像模式。
...
/**
 * @if English
 * Video color space range.
 * @endif
 * @if Chinese
 * 视频颜色范围。
 * @endif
 */
@property(nonatomic, assign) NERtcVideoColorSpaceRange colorSpaceRange;
 ...
 * <br>width 表示视频帧在横轴上的像素，即自定义宽。
 * - 设置为负数时表示采用 maxProfile 档位。
 * - 如果需要自定义分辨率场景，则设置此属性，maxProfile 属性失效。
 * 自定义视频输入 width 和 height 无效，会自动根据 maxProfile 缩放。
 * @endif
 */
...
/**
 * @if Chinese
 * 音效播放进度回调间隔，单位 ms，取值范围为 100~10000, 默认 1000ms
 * @endif
 */
@property (nonatomic, assign) uint64_t progressInterval;
 ...
 * - NO：在直播中不播放该用户的视频流。
 * <br>推流模式为 kNERtcLsModeAudio 时无效。
 * @endif
 */
 ...
 * @endif
 * @if Chinese
 * <br>Y 参数用于设置用户图像的纵轴坐标值。通过 x 和 y 指定画布坐标中的一个点，该点将作为用户图像的左上角。
 * <br>取值范围为 0~1920，若设置为奇数值，会自动向下取偶。
 * <br>用户图像范围如果超出超出画布，调用方法时会报错。
 * @endif
 */
 ...
 * @endif
 * @if Chinese
 * 图层编号，用来决定渲染层级, 取值 0-100 , 0 位于最底层，100 位于最顶层，相同层级的渲染区域按照现有的覆盖逻辑实现（按照数组中顺序进行渲染，index 递增依次往上叠加）。
 * @endif
 */
 ...
 * @endif
 * @if Chinese
 * 画面背景颜色，格式为 256 ✖ 256 ✖ R + 256 ✖ G + B 的和。请将对应 RGB 的值分别带入此公式计算即可。若未设置，则默认为 0。
 * @endif
 */
 ...
 * The field is optional and supports not being set or being set as empty. In this case, the streaming task ID is generated and managed by the SDK, and will be automatically cleared when the user leaves. If manual clearing of the streaming task is required, call the removeLiveStreamTask interface and specify taskID as empty.
 * @endif
 * @if Chinese
 * 自定义的推流任务 ID。字母、数字、下划线组成的 64 位以内的字符串。请保证此 ID 唯一。
 * 该字段为可选，支持不设置或设置为空。在这种情况下，推流任务 ID 由 SDK 生成并管理，并将在用户离开时自动清除。如果需要手动清除推流任务，调用 removeLiveStreamTask 接口，并将 taskID 指定为空即可。 。
 * @endif
 */
...
/**
 * @if English
 * Video color space matrix.
 * @endif
 * @if Chinese
 * 视频颜色空间矩阵。
 * @endif
 */
@property(nonatomic, assign) NERtcVideoColorSpaceMatrix colorSpaceMatrix;

/**
 * @if English
 * Video color space primary.
 * @endif
 * @if Chinese
 * 视频颜色空间主色。
 * @endif
 */
@property(nonatomic, assign) NERtcVideoColorSpacePrimary colorSpacePrimary;

/**
 * @if English
 * Video color space transfer.
 * @endif
 * @if Chinese
 * 视频颜色空间传输。
 * @endif
 */
@property(nonatomic, assign) NERtcVideoColorSpaceTransfer colorSpaceTransfer;
 ...
 * @endif
 */
@property (nonatomic, copy, nullable) NSString* lbsServer;
 ...
 * @if Chinese
 * 是否使用 ipv6 地址(默认为 NO)。
 * @deprecated 该参数已废弃，请使用 NERtcEngineContext 中的 ipAddrMode 字段替代。
 * @endif
 */
...
/**
 低延时直播参数
 */
NERTC_EXPORT @interface NERtcLowLatencyOptions : NSObject

/**
 * 是否开启低延时直播
 */
@property (nonatomic, assign) BOOL enableLowLatency;
/**
 * 直播的超时时间，单位：秒，默认值 60，最小值 30
 */
@property (nonatomic, assign) NSUInteger playTimeout;
/**
 * 直播视频流画布
 */
@property (nonatomic, nullable, strong) NERtcVideoCanvas *playCanvas;
...
/**
 * 自定义信息，最长支持 127 个字符, 若超过 127 个字符，该参数无效。
 */
@property (nonatomic, nullable, copy) NSString *customInfo;

/**
 * @if Chinese
 * 权限密钥。能控制通话时长及媒体权限能力。如果是高级 Token 鉴权场景，用户加入时需要设置权限密钥，该参数的值由您的业务服务器提供。
 * @endif
 */
@property (nonatomic, nullable, copy) NSString *permissionKey;

@property (nonatomic, nullable, strong) NERtcLowLatencyOptions *lowLatencyOptions;
...
/**
 * 视频相关信息，仅 mediaType 为 video\screenshare\之一时有效，其他情况为 nil
 */
@property (nonatomic, strong) NERtcPreDecoderVideoInfo *videoInfo;

/**
 * 音频相关信息，仅 mediaType 为 audio 时有效，其他情况为 nil
 */
...
/**
 * 每一个 nal 长度，按顺序记录每一个 nal 的数据长度，长度之和不能超过 nalData 的长度
 */
...
/**
 * 音频数据音量标记，有效值[0,100]，用于后台 ASL 选路时参考。默认 100。
 */
...
/**
 * @if English
 * Data need to be sent or received.
 * @endif
 * @if Chinese
 * 需要发送或接收的数据。
 * @endif
...
/**
 * @if English
 * The capacity of the data that needs to be sent or received.
 * @endif
 * @if Chinese
 * 需要发送或接收的数据大小。
 * @endif
...
#pragma mark - push stream params

NERTC_EXPORT @interface NERtcStreamingRoomInfo : NSObject

/**
* @if English
* The unique identifier ID for a user, represented as a string of digits. Each user in a room must have a unique uid.
* <br>This uid serves as the user's ID within your application and should be managed and maintained by your application's backend server.
* @endif
* @if Chinese
* 用户的唯一标识 ID，为数字串，房间内每个用户的 uid 必须是唯一的。
* <br>此 uid 为用户在您应用中的 ID，请在您的业务服务器上自行管理并维护。
* @endif
*/
@property (nonatomic, assign) uint64_t uId;
...
* @if English
* The room name.
* <br><ul><li>String format, with a length ranging from 1 to 64 bytes.<li>Supports the following 89 characters: a-z, A-Z, 0-9, space, !#$%&()+-:;≤.,>? @[]^_{|}~”
* @endif
* @if Chinese
* 房间名
* <br><ul><li>字符串格式，长度为 1 ~ 64 字节。<li>支持以下 89 个字符：a-z, A-Z, 0-9, space, !#$%&()+-:;≤.,>? @[]^_{|}~”
* @endif
*/
@property (nonatomic, copy) NSString *channelName;
/**
* @if English
* <br>Security authentication signature (NERTC Token), which can be set as follows: <ul>
<li>null. This is allowed in debug mode; however, it's not secure. It's recommended to switch back to the default secure mode in the CommsEase user console before launching your product officially.</li>
<li>The obtained NERTC Token. Under secure mode, it must be set to the acquired Token. Failure to provide the correct Token will prevent entry into the room. Using the secure mode is recommended.</li> </ul>
* @endif
* @if Chinese
* <br>安全认证签名（NERTC Token），可以设置为：<ul><li>null。调试模式下可设置为 null。安全性不高，建议在产品正式上线前在网易云信控制台中将鉴权方式恢复为默认的安全模式。<li>已获取的 NERTC Token。安全模式下必须设置为获取到的 Token。若未传入正确的 Token 将无法进入房间。推荐使用安全模式。
* @endif
*/
@property (nonatomic, copy) NSString *token;
...
NERTC_EXPORT @interface NERtcPushStreamingConfig : NSObject

/**
* @if English
* Specifies the CDN streaming address for the audio/video stream, which is the playback address on the live streaming CDN.
* @endif
* @if Chinese
* 指定该音视频流的 CDN 推流地址，即直播 CDN 上的播放地址。
* @endif
*/
...
/**
* @if English
* Information of the room for stream pushing.
* @endif
* @if Chinese
* 推流的房间信息。
* @endif
*/
@property (nonatomic, strong, nullable) NERtcStreamingRoomInfo *streamingRoomInfo;

@end

NERTC_EXPORT @interface NERtcPlayStreamingConfig : NSObject

/**
* @if English
* The URL for stream pulling, for example, rtmp://test.url.
* This URL can be set to the playUrl parameter returned by the server API when creating a room in the Netease Cloud Communication live streaming product.
* @endif
* @if Chinese
* 直播拉流地址，例如 rtmp://test.url。
* 此处的拉流地址可设置为网易云信直播产品中服务端 API 创建房间的返回参数 playUrl。
* @endif
*/
@property (nonatomic, copy) NSString *streamUrl;
...
/**
* @if English
* Playback delay in milliseconds. The default value is 2000.
* @endif
* @if Chinese
* 播放延时，单位为毫秒。默认值为 2000。
* @endif
*/
@property (nonatomic, assign) uint32_t playOutDelay;

/**
 * 
 * @if English
 * The timeout for reconnecting to the stream, in milliseconds. The default value is 30000 milliseconds.
 * @endif
 * @if Chinese
 * 重连超时时间，单位为毫秒。默认为30000毫秒。
 * @endif
 */
@property (nonatomic, assign) uint32_t reconnectTimeout;

/**
 * @if English
 * Whether to enable audio playout for the stream.
 * @endif
 * @if Chinese
 * 是否启用音频播放。
 * @endif
 */
@property (nonatomic, assign) BOOL enableAudioPlay;

/**
 * @if English
 * Whether to enable video playout for the stream.
 * @endif
 * @if Chinese
 * 是否启用视频播放。
 * @endif
 */
@property (nonatomic, assign) BOOL enableVideoPlay;

/**
 * @if English
 * Whether to pause pull streaming.
 * @endif
 * @if Chinese
 * 是否暂停拉流。
 * @endif
 */
@property (nonatomic, assign) BOOL pausePlayStreaming;
@end

NERTC_EXPORT @interface NERtcASRCaptionConfig : NSObject
 ...
 *
 *【源语言支持--常见语言】
 * AUTO-中英文自动识别；ar-阿拉伯语；de-德语；en-英语；es-西班牙语；fr-法语；hi-印地语；id-印度尼西亚语；it-意大利语；
 * ja-日语；ko-韩语；nl-荷兰语；pt-葡萄牙语；ru-俄语；th-泰语；vi-越南语；zh-简体中文；
 *【源语言支持--汉语方言】
 * db-东北话；nan-闽南语；sc-四川话；yue-粤语；zj-浙江话；
 *【源语言支持--非常见语言】
 * bn-孟加拉语；fa-波斯语；he-希伯来语；km-高棉语；lo-老挝语；ms-马来语；tl-菲律宾语；tr-土耳其语；
 * 
 * @endif
 ...
 *
 *【源语言支持--常见语言】
 * ar-阿拉伯语；de-德语；en-英语；es-西班牙语；fr-法语；hi-印地语；id-印度尼西亚语；it-意大利语；
 * ja-日语；ko-韩语；nl-荷兰语；pt-葡萄牙语；ru-俄语；th-泰语；vi-越南语；zh-简体中文；
 *【源语言支持--汉语方言】
 * db-东北话；nan-闽南语；sc-四川话；yue-粤语；zj-浙江话；
 *【源语言支持--非常见语言】
 ...
 * @endif
 */
@property(nonatomic, strong) NSArray<NSString *> *srcLanguages;
 ...
 *
 * 支持的语言列表：
 * AUTO-中英文自动识别；CN-简体中文；VI-越南语；EN-英语；ID-印度尼西亚语
 * IT-意大利语；ES-西班牙语；JA-日语；PT-葡萄牙语；ΚΟ-韩语；FR-法语
 * RU-俄语；DE-德语；AR-阿拉伯语泰语；TH-泰语；KH-高棉语；LA-老挝语
 *
 * @endif
 */
@property (nonatomic, copy) NSString *dstLanguage;
 ...
 * @endif
 */
@property(nonatomic, strong) NSArray<NSString *> *dstLanguages;


/**
 * @if Chinese
 * 当源语言与设置的目标语言相同时，是否仍要翻译
 * 默认为 false
 * @endif
 */
@property (nonatomic, assign) BOOL needTranslateSameLanguage;

@end


NERTC_EXPORT @interface NERtcAsrCaptionResult : NSObject
 ...
 * @endif
 */
@property (nonatomic, assign) bool haveTranslation;
/**
 * @if Chinese
 * 翻译后的字幕内容。
 * @endif
...
NERTC_EXPORT @interface NERtcMultiPathOption : NSObject
/**
 * @if Chinese
 * 是否开启多路径
 * @endif
 ...
 * @endif
 */
@property (nonatomic, assign) NERtcMultiPathMediaMode mediaMode;
 ...
 * @endif
 */
@property (nonatomic, assign) uint32_t badRttThreshold;
 ...
 * @endif
 */
@property (nonatomic, assign) bool redAudioPacket;
 ...
 * @endif
 */
@property (nonatomic, assign) bool redAudioRtxPacket;
 ...
 * @endif
 */
@property (nonatomic, assign) bool redVideoPacket;
 ...
 * @endif
 */
@property (nonatomic, assign) bool redVideoRtxPacket;
