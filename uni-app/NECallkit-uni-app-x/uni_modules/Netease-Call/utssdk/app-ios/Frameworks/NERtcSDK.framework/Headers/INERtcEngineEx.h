 ...
 * @note
 * - 该接口已经废弃，请使用 setVideoStreamLayerCount 接口。
 ...
 *  <tr>
 *      <td>layerCount</td>
 *      <td>NERtcVideoStreamLayerCount</td>
 *      <td>设置视频流模式：<ul><li>kNERtcVideoStreamLayerCountOne：开启单流模式。<li>kNERtcVideoStreamLayerCountTwo：开启双流模式。</td> <li>kNERtcVideoStreamLayerCountThree：开启三流模式。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 * [[NERtcEngine sharedEngine] setVideoStreamLayerCount:kNERtcVideoStreamLayerCountTwo];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @endif
 */
- (int)setVideoStreamLayerCount:(NERtcVideoStreamLayerCount)layerCount;
/**
 * @if English
 ...
 * @since V4.6.10
 * @param uidArray 可订阅自己音频的用户 uid 列表。
 ...
 * @note The audio-only SDK disables this API. If you need to use the API, you can download the standard SDK from the official website of CommsEase and replace the audio-only SDK.
 * @param mode The video display mode. For more information, see {@link NERtcVideoRenderScaleMode}.
 * @param streamType The video stream type. For more information, see {@link NERtcStreamChannelType}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置画布中本地视频画面的显示模式。
 ...
 * @endif
 */
- (int)setLocalRenderScaleMode:(NERtcVideoRenderScaleMode)mode streamType:(NERtcStreamChannelType)streamType;
 ...
 * @endif
 */
- (int)setRemoteRenderScaleMode:(NERtcVideoRenderScaleMode)mode forUserID:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType;
 ...
 * @if Chinese 
 * 设置本地视频画面的旋转模式。
 * <br>该接口用于设置本地视频画面在本地和远端设备上的旋转模式，可以指定本地画面和系统设备的横屏/竖屏模式一致、或者和 App UI 的横屏/竖屏模式一致。
 * @note 
 * - 请在加入房间之前调用此接口。
 ...
 * 开始进行音频 dump。
 * @note 该方法可在加入房间前后调用。
 * @param type 音频 dump 类型。详细信息请参考 {@link NERtcAudioDumpType}。
 * @return 操作返回值，成功则返回 0
 * @endif
 */
...
/**
 * @if English
 * Sets the video dump type.
 * @param type the video dump type.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置视频 dump 类型。
 * @param type 视频 dump 类型。
 * @return 操作返回值，成功则返回 0。
 * @endif
 */
- (int)setVideoDump:(NERtcVideoDumpType)type;
...
/**
 * @if English
 * Query the supportive level of a function provided by NERTC SDK on the current device.
 *
 * @since v5.6.30
 * @par Scene
 * Taking the virtual background as an example, before the user jumps to the UI to the live broadcast, you can call the interface to query device about the support level of the virtual background.
 * - kNERtcFeatureSupportFull: The device supports the virtual background function and can directly enable the virtual background.
 * - kNERtcFeatureSupportHardwareLimit: Due to equipment hardware or system reasons, the device does not support virtual background functions.
 * - kNERtcFeatureSupportPerformanceLimit: Because the equipment performance is insufficient, it does not support virtual background functions, but it can be forced to open. For example, low-performance devices are forced to enable the virtual background function by setting the \ref nertc::IRtcEngineEx::enableVirtualBackground "enableVirtualBackground" interface in `YES` force mode.
 *
 * @par When to use
 * Please call this interface after the engine is initialized.
 * @return
 * - 0: Successful interface execution.
 * - Other: interface execution failure.
 * @endif
 * @if Chinese
 ...
 * - kNERtcFeatureSupportPerformanceLimit: 由于设备性能不足，不支持虚拟背景功能，但可以强制开启。例如，低性能设备通过设置 {@link INERtcEngineEx#getFeatureSupportedType: getFeatureSupportedType} 接口的 `force` 为 `YES` 强制开启虚拟背景功能。
 *
 * @par 调用时机
 * 请在初始化引擎之后，调用此接口。
 * @return
...
/**
 * @if English
 * Obtain the local network connection type
 *
 * @since v5.6.40
 * @par When to use
 * Please call this interface after the engine is initialized.
 * @return NERtcNetworkConnectionType The network type
 * @endif
 *
 * @if Chinese
 * 获取本地网络连接类型。
 ...
 * @par 调用时机
 * 请在初始化引擎之后，调用此接口。
 * @return NERtcNetworkConnectionType 网络类型
 * @endif
 */
...
/**
 * @if English
 * Sets the multipath option.
 * <br>This method is used to set whether to enable the multipath feature. After you enable the multipath feature, the SDK can use multiple network paths to transmit media data, so as to improve the quality and reliability of media transmission.
 * @note You must call this method before you join a room.
 * @since V5.9.10
 * @param multi_path_option The multipath option. For more information, see {@link LiteSDKMultiPathOption}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置多路径选项。
 * <br>该方法用于设置是否开启多路径功能，开启多路径功能后，SDK 可以使用多条网络路径传输媒体数据，从而提升媒体传输的质量和可靠性。
 * @note 请在加入房间之前调用此接口。
 * @since V5.9.10
 * @param multi_path_option 多路径选项。详细信息请参考 {@link LiteSDKMultiPathOption}。
 * @return 操作返回值，成功则返回 0
 * @endif
 */
- (int)setMultiPathOption:(NERtcMultiPathOption *)multiPathOption;
 ...
 * @endif
 * @if Chinese
 * @deprecated 该方法已经废弃，请使用  {@link INERtcEngineEx#setExternalVideoSource:streamType:} 方法代替。注意，如果已经调了此方法，将会导致再 {@link INERtcEngine#setExternalVideoSource:streamType:} 方法失效。
 * 开启或关闭外部视频源数据输入。
 * <br>
 ...
 * - 如果调用此方法之前调过废弃方法  {@link INERtcEngineEx#setExternalVideoSource:isScreen:} ，此方法会不生效。
 * @par 参数说明
 * <table>
 *  <tr>
 *      <th>**参数名称**</th>
 ...
 * @endif
 * @if Chinese 
 * @deprecated 此方法已废弃，请使用方法  {@link INERtcEngineEx#pushExternalVideoFrame:streamType:} 代替。
 * 推送外部视频帧。
 ...
 * - 纯音频 SDK 禁用该接口，如需使用请前往云信官网下载并替换成视频 SDK。
 * @param frame 外部视频帧的数据信息。详细信息请参考 {@link NERtcVideoFrame}。
 * @param streamType 视频流通道类型，默认为主流 kNERtcStreamChannelTypeMainStream。
 * @return
 * - 0：方法调用成功；
 * - 其他：方法调用失败。
 * @endif
 ...
 * @if English
 * Sets the local view.
 * <br>This method is used to set the display information about the local video. The method is applicable only to local users.
 * Remote users are not affected. Apps can call this API operation to associate with the view that plays local video streams.
 * During application development, in most cases, before joining a room, you must first call this method to set the local video
 * view after the SDK is initialized.
 ...
 * @param canvas The video canvas. For more information, see {@link NERtcVideoCanvas}. If you want to delete the canvas, you can
 * set the value to nil.
 * @param streamType stream type, default is kNERtcStreamChannelTypeMainStream.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置本地用户视图。
 ...
 *      <td>streamType</td>
 *      <td>{@link NERtcStreamChannelType}</td>
 *      <td>视频流通道类型，默认为主流 kNERtcStreamChannelTypeMainStream。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 ...
 * @endif
 */
- (int)setupLocalVideoCanvas:(nullable NERtcVideoCanvas *)canvas streamType:(NERtcStreamChannelType)streamType;
 ...
 * @param userID The ID of a remote user.
 * @param canvas The video canvas. For more information, see {@link NERtcVideoCanvas}. To delete the canvas setting, set the value to nil.
 * @param streamType The stream type. For more information, see {@link NERtcStreamChannelType}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置远端用户的视频辅流画布。
 ...
 *      <td>流类型。详细信息请参考 {@link NERtcStreamChannelType}。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 ...
 * @endif
 */
- (int)setupRemoteVideoCanvas:(nullable NERtcVideoCanvas *)canvas forUserID:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType;
/**
 * @if English
 ...
 * @param subscribe The option whether to unsubscribe from local video streams.
 * @param userID The ID of a specified user.
 * @param streamType The type of the video streams. For more information, see {@link NERtcStreamChannelType}.
 * @param subscribeType The type of the video streams. For more information, see {@link NERtcRemoteVideoStreamType}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 订阅或取消订阅指定远端用户的视频主流。
 ...
 *      <td>streamType</td>
 *      <td>{@link NERtcStreamChannelType}</td>
 *      <td>视频流类型。详细信息请参考 {@link NERtcStreamChannelType}。</td>
 *  </tr>
 *  <tr>
 *      <td>subscribeType</td>
 *      <td>{@link NERtcRemoteVideoStreamType}</td>
 ...
 * @endcode
 * @par 相关接口
 * 若您希望订阅指定远端用户的视频辅流，请调用 {@link INERtcEngineEx#subscribeRemoteVideo:forUserID:streamType:subscribeType:} 方法。
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 *      - 30001（kNERtcErrFatal）：状态错误，比如引擎尚未初始化。
 ...
 * @endif
 */
- (int)subscribeRemoteVideo:(BOOL)subscribe forUserID:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType subscribeType:(NERtcRemoteVideoStreamType)subscribeType;
 ...
 * @if Chinese 
 * 设置音乐文件的播放位置。
 * <br>该方法可以设置音频文件的播放位置，这样您可以根据实际情况播放文件，而非从头到尾播放整个文件。
 * @param position 音乐文件的播放位置，单位为毫秒。
 * @return 操作返回值，成功则返回 0
 * @endif
 */
 ...
 * <br>请在加入房间后调用该方法。
 * @param effectId 指定音效的 ID。每个音效均有唯一的 ID。
 * @param volume 音效发送音量。范围为 0~100，默认为  100，表示原始音量。
 * @return 操作返回值，成功则返回 0
 * @endif
 */
 ...
 * <br>请在加入房间后调用该方法。
 * @param effectId 指定音效的 ID。每个音效均有唯一的 ID。
 * @param volume 音效播放音量。范围为 0~100，默认为 100。
 * @return 操作返回值，成功则返回 0
 * @endif
 */
 ...
 * @param data 数据指针。
 * @param len 待拉取音频数据的字节数，单位为 byte。建议音频数据的时长至少为 10 毫秒，数据长度不能超过 7680 字节。<br>计算公式为： len = sampleRate/1000 × 2 × channels × 音频数据时长（毫秒）。
 * @return 操作返回值，成功则返回 0
...
* @param[in] mode 范围语音模式，包括所有人和仅小队两种模式。具体请参考 {@link NERtcRangeAudioMode}。
* @return
* - 0: 方法调用成功
* - 其他: 调用失败
*/
...
* @param teamId 小队 ID, 有效值: >=0。若 team_id = 0，则房间内所有人（不论范围语音的模式是所有人还是仅小队）都可以听到该成员的声音。
* @return
* - 0: 方法调用成功
* - 其他: 调用失败
*/
...
/**
  ...
  * 距离衰减模式，具体请参考 {@link #NERtcDistanceRolloffModel}，默认值为 kNERtcDistanceRolloffNone。
 * @return
 * - 0: 方法调用成功
 * - 其他: 调用失败
 */
 ...
 * @par 调用时机
  ...
  * 通过 info 参数设置空间音效中说话者和接收者的空间位置信息。#NERtcPositionInfo 的具体参数说明如下表所示。
    ...
    *      <td>说话者的位置信息，三个值依次表示 X、Y、Z 的坐标值。默认值{0,0,0} </td>
    *  </tr>
    *  <tr>
    ...
    *  <tr>
    *      <td>head_position</td>
    *      <td>接收者的位置信息，三个值依次表示 X、Y、Z 的坐标值。默认值{0,0,0} </td>
    *  </tr>
    *  <tr>
 ...
 * @param roomProperty 房间属性，具体请参考 #NERtcSpatializerRoomProperty
 * @return
 * - 0: 方法调用成功
 * - 其他: 调用失败
 */
 ...
 * 设置空间音效的渲染模式
  ...
  * @note 
  * 请先调用 {@link #enableSpatializer:applyToTeam:} 接口启用空间音效，再调用本接口。
 * @param mode 渲染模式，具体请参考 #NERtcSpatializerRenderMode，默认值 #kNERtcSpatializerRenderBinauralHighQuality
 * @return
 * - 0: 方法调用成功
 * - 其他: 调用失败
 */
 ...
 * @param enabled 是否打开 3D 音效算法功能，默认为关闭状态。
 * @param apply_to_team 是否仅本小队开启 3D 音效。true: 仅仅和接收端同一个小队的人有 3D 音效；false: 接收到所有的语音都有 3d 音效
 * - YES：: 开启空间音效
 * - NO: 关闭空间音效
 * @return
 * - 0: 方法调用成功
...
/**
     ...
     * @since V5.5.10
     * @param[in] type 音频流类型。
     * @param[in] uidArray 不订阅此 用户 uid 列表 的音频。
...
/**
     ...
     *  - 对于调用接口时不在房间的 uid 不生效。
     * @since V5.5.10
     * @param[in] uid_array 只订阅此 用户 uid 列表 的音频。
 ...
 * @note 当 kNERtcKeyAutoSubscribeData 默认打开时，用户不能手动修改数据通道订阅状态
 ...
 * @if English
 * Sets the format of the recording and mixing audio.
 * <br>The method sets the format of the audio frames returned by onNERtcEnginePlaybackAudioFrameBeforeMixingWithUserID.
 * - You can set or modify the format before or after you join a room. leaveChannel is reset to nil.
 * - You can set only the sample rate.
 * - If you do not use the method to set the format, the default value of the sample rate supported by the SDK is returned.
 * @param format The sample rate and the number of channels returned by onNERtcEnginePlaybackAudioFrameBeforeMixingWithUserID. A value of nil is allowed. The default value is nil, which indicates that the original format of the audio file is used. For more information, see {@link NERtcAudioFrameRequestFormat}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 设置录制和播放声音混音后的数据格式。
 * <br>该方法设置 {@link NERtcEngineAudioFrameObserver#onNERtcEnginePlaybackAudioFrameBeforeMixingWithUserID:frame:channelId:} 回调的声音格式。
 * - 该方法在加入房间前后均可设置或修改，{@link INERtcEngine#leaveChannel} 后重置为空。
 * - 目前只支持设置采样率。
 * - 未调用该接口设置数据格式时，回调中的采样率返回 SDK 默认值。
 * @param format 指定 {@link NERtcEngineAudioFrameObserver#onNERtcEnginePlaybackAudioFrameBeforeMixingWithUserID:frame:channelId:} 中返回数据的采样率和数据的通道数。允许传入 nil，默认为 nil，表示使用音频的原始格式。详细信息请参考 {@link NERtcAudioFrameRequestFormat}。
 * @return
 * - 0: 方法调用成功；
 * - 其他: 方法调用失败。
* @endif
 ...
 * - {@link NERtcEngineAudioFrameObserver#onNERtcEnginePlaybackAudioFrameBeforeMixingForPlayStreaming:playStreamId:frame:}：获取指定直播拉流混音前的音频辅流数据。
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @if Chinese
 * 注册视频数据观察对象。
 * <br>通过此接口可以设置视频数据采集回调对象，可以用于第三方美颜等操作。
 * <br>设置空指针，表示不再监听视频数据采集回调。
 * @since V4.6.40
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法在加入房间前后均可调用。
 * @par 示例代码
...
/**
 * @if English
 * Adds a remote video frame observer.
 * <br>After you call the method, the SDK triggers the onNERtcEngineRemoteVideoFrame callback to report the video frame of the remote user.
 * @param videoframeObserver The video frame observer. <br> If you pass in NULL, you cancel the registration and clear the settings . For more information, see {@link NERtcEngineVideoFrameObserver}.
 * @return The value returned. A value of 0 indicates that the operation is successful.
 * @endif
 * @if Chinese
 * 添加远端视频帧观察者。
 * <br>通过此接口可以实现设置远端用户的视频帧回调。
 * @since V5.9.5
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法在加入房间前后均可调用。
 * @par 示例代码
 * @code
 * //开启监听远端视频数据回调
 * [[NERtcEngine sharedEngine] addRemoteVideoFrameObserver:self];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @endif
 */
- (int)addRemoteVideoFrameObserver:(nullable id<NERtcEngineVideoFrameObserver>)videoframeObserver;
 ...
 * @param customIdentify 自定义标识，比如产品或业务类型，如不需要填 null
 * @param param          参数键值对，参数值支持 String 及 java 基本类型(int 、bool....)， 如不需要填 null
 * @return 操作返回值，成功则返回 0
 * @endif
 */
 ...
 * @if Chinese 
 * 更新媒体流转发的目标房间。
 * <br>成功开始跨房间转发媒体流后，如果您希望将流转发到多个目标房间，或退出当前的转发房间，可以调用该方法。
 ...
 * @par 示例代码
 * @code
 * [channel pauseChannelMediaRelay];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @since V5.8.25
 * @par 使用前提
 * 请在调用 {@link INERtcChannel#pauseChannelMediaRelay:} 方法开启跨房间媒体流转发之后调用此接口。
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法仅可在加入房间后调用。
 * @par 示例代码
 * @code
 * [channel resumeChannelMediaRelay];
 * @endcode
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @if Chinese
 * 创建一个 IRtcChannel 对象
 * @param[in] channelName      房间名。设置相同房间名称的用户会进入同一个通话房间。字符串格式，长度为 1~ 64 字节。支持以下 89 个字符：a-z, A-Z, 0-9, space, !#$%&()+-:;≤.,>?@[]^_{|}~”
...
/**
 * @if English
 * Enable or disable the virtual background feature.
     * - After enabling the virtual background function, you can use a custom image to replace the original background picture of the local user.
     * - After replacement, all users in the channel can see the custom picture.
 * @note
 * - To see if you successfully turn on the virtual background or get the cause of the error, please use {@link NERtcEngineDelegateEx#onNERtcEngineVirtualBackgroundSourceEnabled:reason:} to implement.
  * - It is recommended that you use this feature in the following conditions:
  *    - Devices with high-definition cameras are used, and the lighting in the environment is even.
  *    - The captured video images are neat, with the user's portrait being half-length and essentially unobstructed, and the background is a solid color different from the user's clothing.
  * - Videos in Texture format, or videos obtained from a custom video source via the Push method, do not support setting a virtual background.
  * - If you have set a custom background image, the NERTC SDK will scale or adjust the image to some extent to fit the video capture resolution without distorting the content of the background image.
 * @since V5.6.30
 * @param enable Whether to enable the virtual background.
     * - true：Enable.
     * - false: Disable.
 * @param backData Custom background picture. For details, please refer to {@link NERtcVirtualBackgroundSource}.
 * @param force Whether to enforce to enable this feature, even on a low-performance device. And YES indicates enable.
 * @return
 * - 0: Method call success.
 * - Other: Method call fails.
 * @endif
 * @if Chinese
 * 开启或关闭虚拟背景。
 ...
 * @note
 * - 如需查看是否成功开启虚拟背景，或获取出错原因，请通过 {@link NERtcEngineDelegateEx#onNERtcEngineVirtualBackgroundSourceEnabled:reason:} 回调实现。
  * - 建议您在满足以下条件的场景中使用该功能：
  *    - 采用高清摄像的设备，环境中的光线均匀。
 ...
 * - false: 关闭。
 * @param backData 自定义背景图片。详细信息请参考 {@link NERtcVirtualBackgroundSource}。
 * @param force 为 YES 时，低性能的设备，也支持强制开启虚拟背景。
 * @return
 * - 0：方法调用成功。
 * - 其他：方法调用失败。
 * @endif
 */
- (int)enableVirtualBackground:(BOOL)enable backData:(NERtcVirtualBackgroundSource *_Nullable)backData force:(BOOL)force;
 ...
 * audioFrame.payloadType = 数据类型，参考枚举 NERtcPayLoadType
...
#pragma mark - push streaming

/**
* @if English
* Start to stream pushing.
* <br>
* Through this method, you can implement pushing an RTMP stream to a designated address.
* @since V4.6.420
* @par When to use
* Please call this method after initialization.
* @note
* - This method is mutually exclusive with the joinChannel.
* @par Parameters
* <table>
*  <tr>
...
*  <tr>
*      <td>config</td>
*      <td>NERtcPushStreamingConfig</td>
*      <td>Parameters for stream pushiong, see NERtcPushStreamingConfig.</td>
*  </tr>
* </table>
 @par Example
 * @code
 ...
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 *         - 30003 (kNERtcErrInvalidParam): Inappropriate parameter setting.
 *         - 30005 (kNERtcErrInvalidState): The operation is not supported in the current state, such as when the engine has not been initialized yet.
 * @endif
* @if Chinese
* 开始推流。
* <br>
...
*      <td>config</td>
*      <td>NERtcPushStreamingConfig</td>
*      <td>推流参数，参考 {@link NERtcPushStreamingConfig}</td>
*  </tr>
* </table>
 @par 示例代码
 * @code
...
* @if English
* Stop stream pushing.
* <br>Through this method, you can stop the current RTMP stream pushing.
* @since V4.6.420
* @par Prerequisite
* It is recommended to call this method after initiating the stream pushing via the {@link INERtcEngineEx#startPushStreaming:}.
* @par When to use
* Please call this method after the engine initialization, and it can be called after the startPushStreaming commences the streaming process.
* @note
* - This method, when invoked during individual streaming, will exit the current RTC room.
* - In scenarios such as PK live connections, invoking it will only halt the RTMP stream push without leaving the current RTC room.
* @par Example
* @code
...
* @if Chinese
* 停止推流。
* <br>通过此接口可以停止当前正在进行的 rtmp 推流。
* @since V4.6.420
* @par 使用前提
* 建议在通过 {@link INERtcEngineEx#startPushStreaming:} 接口开启推流后调用此方法。
* @par 调用时机
...
#pragma mark - play streaming

// 添加注释
/**
 * @if English
 * Setup the canvas for playing the RTMP stream.
 * @since V5.8.25
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 * - There is no need to join an RTC room when calling this method.
 * @par Parameters
 * <table>
 *  <tr>
 *      <th>**Parameter name**</th>
 ...
 *      <td>streamId</td>
 *      <td>NSString</td>
 *      <td>The ID of the stream to be played.</td>
 *  </tr>
 *  <tr>
 *      <td>canvas</td>
 *      <td>NERtcVideoCanvas</td>
 *      <td>The canvas to render the video stream.</td>
 *  </tr>
 * </table>
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] setupPlayStreamingCanvas:streamId canvas:canvas];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */
- (int)setupPlayStreamingCanvas:(NSString *)streamId canvas:(NERtcVideoCanvas *)canvas;

/**
* @if English
* Start rtmp stream playing.
* <br>Through this method, you can start the current RTMP stream playing.
* @since V5.8.25
* @par When to use
* Please call this method after the engine initialization.
* @note
* - There is no need to joiin an RTC room when calling this method.

* @par Example
* @code
* [[NERtcEngine sharedEngine] startPlayStreaming:streamId playConfig:config];
* @endcode
* @return
* - 0 (kNERtcNoError): A successful response.
* - Others: An error response.
...
* @endif
*/
- (int)startPlayStreaming:(NSString *)streamId playConfig:(NERtcPlayStreamingConfig*)config;

/**
* @if English
* Stop rtmp stream playing.
* <br>Through this method, you can stop the current RTMP stream playing.
* @since V5.8.25
* @par Prerequisite
* It is recommended to call this method after initiating the stream pushing via the {@link INERtcEngineEx#startPlayStreaming:}.
* @par When to use
* Please call this method after the engine initialization, and it can be called after the startPlayStreaming commences the streaming process.
* @note
* - There is no need to joiin an RTC room when calling this method.
* @par Example
* @code
* [[NERtcEngine sharedEngine] stopPlayStreaming];
* @endcode
* @return
* - 0 (kNERtcNoError): A successful response.
* - Others: An error response.
...
* @if Chinese
* 停止播放rtmp流。
* <br>通过此接口可以停止当前正在播放的rtmp流。
* @since V5.8.25
* @par 使用前提
* 建议在通过 {@link INERtcEngineEx#startPlayStreaming:} 接口开启拉流后调用此方法。
* @par 调用时机
* 请在引擎初始化之后调用此接口，且该方法在 startPlayStreaming 开始拉流之后可以调用
* @note
* - 调用该方法时不需要加RTC房间。
* @par 示例代码
* @code
...
* @endif
*/
- (int)stopPlayStreaming:(NSString *)streamId;

/** Pause RTMP stream playing. 
 * * @since V5.9.5
 *
 * @par When to use
 * Please call this method after the engine initialization.
 ...
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] pausePlayStreaming:streamId];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */        
- (int)pausePlayStreaming:(NSString *)streamId;

/** Resume RTMP stream playing
 * * @since V5.9.5
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 ...
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] resumePlayStreaming:streamId];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */
- (int)resumePlayStreaming:(NSString *)streamId;


/**
 * @if English
 * Enable or disable video playout for play streaming.
 * @since V5.9.5
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 ...
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] muteVideoForPlayStreaming:streamId mute:YES];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */
- (int)muteVideoForPlayStreaming:(NSString *)streamId mute:(BOOL)mute;

/**
 * @if English
 * Enable or disable audio playout for play streaming.
 * @since V5.9.5
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 ...
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] muteAudioForPlayStreaming:streamId mute:YES];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */
- (int)muteAudioForPlayStreaming:(NSString *)streamId mute:(BOOL)mute;

#pragma mark - ai relate
 ...
 *  <tr>
 *      <td>config</td>
 *      <td>NERtcASRCaptionConfig</td>
 *      <td>ASR Caption参数，参考 {@link NERtcASRCaptionConfig}</td>
 *  </tr>
 * </table>
 @par 示例代码
 * @code
 * NERtcASRCaptionConfig *asrConfig = [[NERtcASRCaptionConfig alloc] init];
 * asrConfig.srcLanguage = "CH";
 * asrConfig.dstLanguage = "";
 ...
 * @endif
 */
- (int)startASRCaption:(NERtcASRCaptionConfig *)config;
 ...
 * @since V5.6.50
 * @par 使用前提
 * 建议在通过 {@link INERtcEngineEx#startASRCaption:} 接口开启实时字幕后调用此方法。
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法在 startASRCaption 开始实时字幕之后可以调用
 * @par 示例代码
 * @code
 ...
 * @since V5.6.50
 * @par 调用时机
 * 请在引擎初始化之后调用此接口，且该方法在加入房间之后才可以调用
 * @par 参数说明
 * <table>
 *  <tr>
 *      <th>**参数名称**</th>
 ...
 *      <td>dstUserId</td>
 *      <td>uint64_t</td>
 *      <td>目标用户ID，即远端的UserId。默认可以传0，SDK会选择当前房间里的第一个远端用户ID</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 ...
 * @endif
 */
- (int)aiManualInterrupt:(uint64_t)dstUserId;
