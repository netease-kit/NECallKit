 ...
 * @endif
 */
- (void)onNERtcEngineUserVideoStreamDidStart:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType streamProfile:(NERtcVideoProfileType)profile;
 ...
 * @param streamType 视频流类型。
 */
- (void)onNERtcEngineUserVideoStreamDidStop:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType;
...
/**
 * @if English 
 * Occurs when a remote user sends a video frame.
 * @note The audio-only SDK disables this API. If you need to use the API, you can download the standard SDK from the official website of CommsEase and replace the audio-only SDK.
 * @param videoStreamType The video stream type.
 * @param userId The ID of the user who sends the video frame.
 * @param data The video frame data.
 * @param format The video format type. For more information, see {@link NERtc      VideoFormatType}.
 * @param width The width of the video frame.
 * @param height The height of the video frame.
 * @param planeCount The number of planes in the video frame.
 * @param offset The offset of the video frame.
 * @param stride The stride of the video frame.
 * @param rotation The video rotation. For more information, see {@link NERtcVideoRotation}.
 * @endif
 * @if Chinese
 * 远端用户发送视频帧回调。
 * @note 纯音频 SDK 禁用该接口，如需使用请前往云信官网下载并替换成视频 SDK。
 * @param videoStreamType 视频流类型。
 * @param userId 发送视频帧的用户 ID。
 * @param data 视频帧数据。
 * @param format 视频格式类型，详细信息请参考 {@link NERtcVideoFormatType}。
 * @param width 视频帧宽度。
 * @param height 视频帧高度。
 * @param planeCount 视频帧平面数。
 * @param offset 视频帧偏移量。
 * @param stride 视频帧步长。
 * @param rotation 视频旋转角度，详细信息请参考 {@link NERtcVideoRotation}。
 * @endif
 */
...
/**
 * @if English 
 * Occurs when play live streaming.
 * @note The audio-only SDK disables this API. If you need to use the API, you can download the standard SDK from the official website of CommsEase and replace the audio-only SDK.
 * @param data The video frame data.
 * @param format The video format type. For more information, see {@link NERtcVideoFormatType}.
 * @param width The width of the video frame.
 * @param height The height of the video frame.
 * @param planeCount The number of planes in the video frame.
 * @param offset The offset of the video frame.
 * @param stride The stride of the video frame.
 * @param rotation The video rotation. For more information, see {@link NERtcVideoRotation}.
 * @param streamId The ID of the live play stream.
 * @endif
 * @if Chinese
 * 拉流播放视频帧回调。
 * @note 纯音频 SDK 禁用该接口，如需使用请前往云信官网下载并替换成视频 SDK。
 ...
 * @if Chinese
 * 解码前媒体数据回调。
 * - 调用 {@link INERtcEngineEx#setPreDecodeObserver:} 方法注册解码前媒体数据观测器后，SDK 会触发该回调，可以通过返回的用户 UID、媒体数据类型、数据长度等信息对媒体数据自行编码。
 * @since V4.6.29
 * @par 使用前提
...
@protocol NERtcEnginePushStreamingObserver <NSObject>
 @optional
/**
* @if English
* Callback for the result of starting the stream pushing via startPushStreaming.

* @par When to use
* After the streamer successfully initiates the stream push using startPushStreaming, the streamer receives the onNERtcEngineStartPushStreamingWithResult callback.
* @since 4.6.420
...
*      <td>result</td>
*      <td>int</td>
*      <td>The result of startPushStreaming. kNERtcNoError indicates no error, and the operation is successful.</td>
*  </tr>
*  <tr>
*      <td>channelId</td>
*      <td>uint64_t</td>
*      <td>The Room ID for stream pushing.</td>
*  </tr>
* </table>
* @param result
* @endif
...
* @par 触发时机
* 当主播调用 startPushStreaming 成功推流后，主播端收到 onNERtcEngineStartPushStreamingWithResult 回调。
* @since 4.6.420
* @par 参数说明
* <table>
*  <tr>
...
*      <td>result</td>
*      <td>int</td>
*      <td>startPushStreaming 的结果。kNERtcNoError 表示没有错误，成功。</td>
*  </tr>
*  <tr>
*      <td>channelId</td>
*      <td>uint64_t</td>
...
/**
* @if English
* Callback for the result of stopping RTMP streaming via the stopPushStreaming method.
* @par When to use
* After the streamer successfully stops the stream pushing by calling stopPushStreaming, the streamer will receive the onNERtcEngineStopPushStreaming callback.
* @since 4.6.420
* @param result   The result of stopPushStreaming. kNERtcNoError indicates no error, and the operation was successful.
* @endif
* @if Chinese
* stopPushStreaming 停止 RTMP 推流的结果回调。
* @par 触发时机
* 当主播调用 stopPushStreaming 成功停止推流后，主播会收到 onNERtcEngineStopPushStreaming 回调。
* @since 4.6.420
* @param result   stopPushStreaming 的结果。kNERtcNoError 表示没有错误，成功。
* @endif
*/
...
/**
* @if English
* Callback triggered when the streaming status changes to reconnecting during the stream pushing process.
* @par When to use
* During the stream pushing process, if the streaming is interrupted due to network issues or other reasons, and the streaming status changes to reconnecting, the onNERtcEnginePushStreamingChangeToReconnectingWithReason callback will be triggered.
* @since 4.6.420
* @param reason The reason for the streaming interruption.
* @endif
* @if Chinese
* 推流过程中推流状态变为重连中的回调
* @par 触发时机
* 推流过程中由于网络等原因导致推流中断，推流状态变为重连中，会触发 onNERtcEnginePushStreamingChangeToReconnectingWithReason 回调。
* @since 4.6.420
* @param reason 推流中断的原因
* @endif
*/
...
/**
* @if English
* Callback for a successful reconnection during the stream pushing process.
* @par When to use
* If the stream pushing is interrupted due to network issues or other reasons during the process, and after a successful reconnection, the onNERtcEnginePushStreamingReconnectedSuccess callback will be triggered.
* @since 4.6.420
* @endif
* @if Chinese
* 推流过程中重连成功的回调。
* @par 触发时机
* 推流过程中由于网络等原因导致推流中断，重连成功后，会触发 onNERtcEnginePushStreamingReconnectedSuccess 回调。
* @since 4.6.420
* @endif
*/
...
@protocol NERtcEnginePlayStreamingObserver <NSObject>
 @optional

/**
 * @if English
 * Callback for when the play streaming process state changes.
 * @par When to use
 * This callback is triggered when the play streaming process state changes.
 * @param streamId The ID of the stream.
 * @param state The state of the play streaming process. For details, see NERtcLivePlayStreamState.
 * @param error The reason for the state change. For details, see NERtcError.
 * @since 5.9.10
 * @endif
 * @if Chinese
 * 拉流过程中拉流状态改变的回调。
 ...
 * 拉流过程中拉流状态改变时会触发该回调。
 * @param streamId 流 ID。
 * @param state 拉流状态。详见 NERtcLivePlayStreamState。
 * @param error 状态改变的原因。详见 NERtcError。
 * @since 5.9.10
 * @endif
 */

- (void)onNERtcEnginePlayStreamingStateChange:(NSString*)streamId state:(NERtcLivePlayStreamState)state error:(NERtcError)error;

/**
 * @if English
 * Callback for when the play streaming process receives a SEI message.
 * @par When to use
 * This callback is triggered when the play streaming process receives a SEI message.
 * @param streamId The ID of the stream.
 ...
 * @endif
 */
- (void)onNERtcEnginePlayStreamingReceiveSeiMessage:(NSString*)streamId message:(NSString*)message;

/**
 * @if English
 * Callback for when the play streaming process first renders a video frame.
 * @par When to use
 * This callback is triggered when the play streaming process first renders a video frame.
 * @param streamId The ID of the stream.
 * @param timeMs The timestamp of the video frame.
 * @param width The width of the video frame.
 * @param height The height of the video frame.
 * @since 5.9.10
 * @endif
 ...
 * 拉流过程中首次渲染视频帧时会触发该回调。
 * @param streamId 流 ID。
 * @param timeMs 视频帧的时间戳。
 * @param width 视频帧的宽度。
 * @param height 视频帧的高度。
 * @since 5.9.10
 * @endif
 */
- (void)onNERtcEnginePlayStreamingFirstVideoFrameRender:(NSString*)streamId timeMs:(int64_t)time_ms width:(uint32_t)width height:(uint32_t)height;

/**
 * @if English
 * Callback for when the play streaming process first plays an audio frame.
 * @par When to use
 * This callback is triggered when the play streaming process first plays an audio frame.
 * @param streamId The ID of the stream.
 * @param timeMs The timestamp of the audio frame.
 * @since 5.9.10
 * @endif
 * @if Chinese
 * 拉流过程中首次渲染音频帧的回调。
 ...
 * 拉流过程中首次渲染音频帧时会触发该回调。
 * @param streamId 流 ID。
 * @param timeMs 音频帧的时间戳。
 * @since 5.9.10
 * @endif
 */
- (void)onNERtcEnginePlayStreamingFirstAudioFramePlayed:(NSString*)streamId timeMs:(int64_t)time_ms;
...
NERtcEnginePushStreamingObserver,
NERtcEnginePlayStreamingObserver>
 ...
 * @if Chinese
 * 远端用户暂停或恢复发送视频流回调。 
 * <br>当远端用户调用 muteLocalVideo 取消或者恢复发布视频流时，SDK 会触发该回调向本地用户报告远程用户的发流状况。
 ...
 * @if Chinese
 * 已接收到远端视频首帧回调。
 * SDK 收到远端第一帧视频并解码成功时，触发此调用。每次重新调用 enableLocalVideo 开启本地视频采集，也会触发该回调（V5.5.10 版本开始）。App 可在此回调中设置该用户的视频画布。
...
/**
  ...
  * <br>当调用 enableLocalAudio 或 enableMediaPub 重新发布音频流也会触发该回调。
  * @par 参数说明
  * <table>
  *  <tr>
  *      <th>**参数名称**</th>
  ...
  *  <tr>
  *      <td>streamType</td>
  *      <td>see #NERtcAudioStreamType</td>
  *      <td>音频通道类型：<ul><li>kNERtcAudioStreamMain ：音频流主流。<li>kNERtcAudioStreamSub ：音频流辅流。</td>
  *  </tr>
  * </table>
  * @endif
 */
 ...
 * @endif
 * @if Chinese
 * 检测啸叫回调, 调用 JoinChannel 成功后，会一直周期性触发该回调, 不建议在该回调里做频繁的 API 调用和耗时操作。
 * 当声源与扩音设备之间因距离过近时，可能会产生啸叫。NERTC SDK 支持啸叫检测，当检测到有啸叫信号产生的时候，参数 flag 为 YES，无啸叫时参数 flag 为 NO。App 应用层可以在有啸叫时，提示用户静音麦克风，或直接静音麦克风。
 * @note 啸叫检测功能一般用于语音聊天室或在线会议等纯人声环境，不推荐在包含背景音乐的娱乐场景中使用。
 * @param flag 是否检测到啸叫。
 * @endif
 */
 ...
 * @since V5.5.0
 * @param key 功能对应的 key
 * @param param 回调的参数，每个 key 对应的参数，数据类型不一样，需要转换后使用，详情请 [提交工单](https://app.yunxin.163.com/global/service/ticket/create) 联系网易云信技术支持工程师
 * @endif
 */
 ...
 * @param state asr caption 的状态，详细信息请参考 {@link NERtcAsrCaptionState}
 * @param code 具体的错误码，成功为200
 * @param message 错误时的描述信息
 * @endif
 */
 ...
 * 实时字幕状态回调。
 * @par 触发时机
 * 当调用 startASRCaption 成功后，有字幕信息时回调。
 * @note
 * - 如果一个房间内有多个用户选择了不同的翻译语言，那么可能会收到多个翻译结果。
 * - 如果只需要其中部分语言的翻译结果，可以通过 NERtcAsrCaptionResult 结构体里的 translation_language 字段进行过滤。
 * @param results 详细的字幕信息数组，详细信息请参考 {@link NERtcAsrCaptionResult}
 * @endif
 */
 ...
 * - 返回音频数据支持读写。
 * - 有本地音频数据驱动就会回调。
 * <br>允许修改 frame 里 void *data 所指向的内容，不允许修改 format。如果对 format 有要求，请通过 setParameter :  kNERtcKeyObservePlaybackAudioFrameFormat 进行设置。
 * @param frame 音频帧数据，详细信息请参考 {@link NERtcAudioFrame}。
 * @endif
 */
 ...
 * @if English
 * Gets the raw audio data of a specified remote user before audio mixing.
 * <br>After the audio observer is registered, if the audio of streaming is enabled, the SDK triggers this callback when the audio data before audio mixing is captured, and the audio data is returned to the user.
 * @note
 * - The returned audio data is read-only.
 * - You cannot modify the content that void *data pointed to in the frame and the format.
 * @param playStreamId The ID of a  live streaming.
 * @param frame The audio frame data. For more information, see {@link NERtcAudioFrame}.
 * @endif
 * @if Chinese
 * 获取某一路拉流混音前的音频数据。
 * <br>成功注册音频观测器后，如果拉流开启了音频，SDK 会在捕捉到混音前的音频数据时，触发该回调，将音频数据回调给用户。
 * @note
 * - 返回音频数据只读。
 * - 不支持修改 frame 里 void *data 所指向的内容，不支持修改 format。
 * @param playStreamId  直播拉流的流ID。
 * @param frame 音频帧数据，详细信息请参考 {@link NERtcAudioFrame}。
 * @endif
 */
- (void)onNERtcEnginePlaybackAudioFrameBeforeMixingForPlayStreaming:(NSString*)playStreamId frame:(NERtcAudioFrame *)frame;
 ...
 * - 返回音频数据只读。
 * - 有本地音频数据驱动就会回调。
 * 不支持修改 frame 里 void *data 所指向的内容，不允许修改 format。如果对 format 有要求，请通过 setMixedAudioFrameParameters 进行设置
 * @param frame 音频帧数据，详细信息请参考 {@link NERtcAudioFrame}。
 * @endif
 */
