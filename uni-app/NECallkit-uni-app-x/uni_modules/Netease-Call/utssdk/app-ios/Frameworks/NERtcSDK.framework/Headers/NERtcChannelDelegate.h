 ...
 * @endif
 */
- (void)onNERtcChannelUserVideoStreamDidStart:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType streamProfile:(NERtcVideoProfileType)profile;
 ...
 * @endif
 */
- (void)onNERtcChannelUserVideoStreamDidStop:(uint64_t)userID streamType:(NERtcStreamChannelType)streamType;
 ...
 * @if Chinese
 * 远端用户暂停或恢复发送视频流回调。
 * @note 当远端用户调用 muteLocalVideo 取消或者恢复发布视频流时，SDK 会触发该回调向本地用户报告远程用户的发流状况。
 * @note 纯音频 SDK 禁用该接口，如需使用请前往云信官网下载并替换成视频 SDK。
 * @since V4.5.0
 * @param userID 用户 ID，提示是哪个用户的视频流。
 ...
 * @param muted 是否暂停发送视频流。
     * - YES：该用户已暂停发送视频流。
     * = NO：该用户已恢复发送视频流。
 * @param streamType  视频流通道类型，默认为主流 kNERtcStreamChannelTypeMainStream。 #NERtcStreamChannelType
 * @endif
 */
 ...
 * @param width  首帧视频宽，单位为 px。
 * @param height 首帧视频高，单位为 px。
 * @param type 视频流通道类型，默认为主流 kNERtcStreamChannelTypeMainStream。 #NERtcStreamChannelType
 * @endif
 */
 ...
 * <br>如果您调用了设置远端订阅流回退选项 setRemoteSubscribeFallbackOption 接口并将 option 设置 kNERtcStreamFallbackOptionAudioOnly 后，当下行网络环境不理想、仅接收远端音频流时，或当下行网络改善、恢复订阅音视频流时，会触发该回调。
 * @since V4.5.0
 ...
 * <br>通常情况下，SDK 上报的错误意味着 SDK 无法自动恢复，需要 App 干预或提示用户。
 * @since V4.5.0
 ...
 * <br>通常情况下，SDK 上报的警告意味着 SDK 无法自动适配，需要 App 干预或提示用户。
 * @since V4.5.0
