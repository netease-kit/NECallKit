...
本目录提供 Electron 场景的 CallKit UI 示例源码，目录命名与 `NECallKit/Electron/package.json` 的 workspace 定义保持一致：
...
`example-vue3` 包含 Electron demo 与 Vue 3 Electron `call-kit-ui` 源码。CallKit 逻辑层、native addon、SDK 源码和构建产物不在本目录内，由 npm 依赖提供。
...
Demo 的 `package.json` 优先使用 npm 上已发布的 Electron UIKit 版本。`call-kit-ui/` 是本次随 demo 上传的 UIKit 源码；如需要本地源码联调，可临时改用本地 tarball 或本地 path，但不要把 CallKit 逻辑层源码提交到本目录。
